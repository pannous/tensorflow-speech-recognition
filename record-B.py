...
offset=0
global offset
hamming_window = np.hamming(length)  # minimize fourier frequency drain  const
def transform_data(data, offset=0):
  ...
  mel_features, mspec, spec = librosa.feature.mfcc(data, fs=sample_rate, nceps=26)
  ...
  if (i < 20 and numpy.sum(np.abs(data)) < 1000 * width):
	  return None
...
def record():
  ...
  offset = 0
  while True:
    data0=read_microphone(stream)
    data0=transform_data(data0,offset)
